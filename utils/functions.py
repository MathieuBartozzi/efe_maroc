...
# def series_by_sessions(dfi: pd.DataFrame, value_col: str = "moyenne") -> pd.Series:
...
# ) -> pd.DataFrame:
#     """Retourne une DF 1 ligne: colonnes 2023/2024/2025 au format 'rang/total'."""
#     def empty_row():
#         return pd.DataFrame([{"2023": "—", "2024": "—", "2025": "—"}])
...
#     return pd.DataFrame([{
#         "2023": format_rank_over_total(s.get(2023, np.nan), n_by_session.get(2023, np.nan)),
#         "2024": format_rank_over_total(s.get(2024, np.nan), n_by_session.get(2024, np.nan)),
#         "2025": format_rank_over_total(s.get(2025, np.nan), n_by_session.get(2025, np.nan)),
#     }])
...
#     """Carte KPI + mini courbe 23-24-25 + mini tableau rang/total."""
#     if s.isna().all():
#         return

#     v23, v24, v25 = s.get(2023, np.nan), s.get(2024, np.nan), s.get(2025, np.nan)
#     value_str = "—" if pd.isna(v25) else f"{float(v25):.2f}"
#     delta_str = "—" if (pd.isna(v25) or pd.isna(v24)) else f"{float(v25 - v24):+.2f}"
...
# # =========================================================
# # HELPERS (delta établissement vs réseau)
# # =========================================================
...
#                 df_all=df_all, df_etab=df_etab,
#                 exam="BAC", group_col="bloc_epreuve",
#                 sessions=sessions,
#                 exclude_values=None,  # à compléter si besoin
#             )
#         )
...
#     dfx["bloc_u"] = norm_series(dfx["bloc_epreuve"])
#     dfx["epreuve_u"] = norm_series(dfx["epreuve"])
...
#     # ATTENTION: textfont_size=50 est énorme et rend souvent le graphique illisible
#     # -> je réduis à une valeur raisonnable.
